...
# Evita abrir janela do cmd quando o nmap roda
subprocess_flags = {
    "startupinfo": None,
    "creationflags": 0
}
...
# Ícone (imagem de computador fictícia)
try:
    imagem = Image.open("icone_computador.png")  # coloque uma imagem com esse nome na pasta
    imagem = imagem.resize((70, 70), Image.ANTIALIAS)
    img_tk = ImageTk.PhotoImage(imagem)
    label_img = Label(janela, image=img_tk)
    label_img.pack(pady=5)
except:
    pass

# Interface
Label(janela, text="Unidade:").pack(pady=(5, 0))
combo_rede = ttk.Combobox(janela, values=list(subredes_unidades.keys()), width=30)
combo_rede.pack()
combo_rede.set("Arouche")

var_agressivo = BooleanVar()
Checkbutton(janela, text="Modo Agressivo", variable=var_agressivo).pack(pady=5)
